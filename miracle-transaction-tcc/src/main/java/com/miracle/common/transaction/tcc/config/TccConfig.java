...
public class TccConfig {
     ...
     * 模块名称
     */
	@Value("${miracle.transaction.tcc.moduleName}")
     ...
     */
	@Value("${miracle.transaction.tcc.serializer}")
    private String serializer = "protostuff";

    /**
     * 回滚队列大小
     */
	@Value("${miracle.transaction.tcc.coordinatorQueueMax}")
    private int coordinatorQueueMax = 5000;
	
	/**
	 * 异步补偿核心线程数
	 */
	@Value("${miracle.transaction.tcc.asyncCompensateThreadCore}")
	private int asyncCompensateThreadCore = 2;
	
	/**
	 * 异步补偿最大线程数
	 */
	@Value("${miracle.transaction.tcc.asyncCompensateThreadMax}")
	private int asyncCompensateThreadMax = 4;
	
    /**
     * 监听回滚队列线程数
     */
	@Value("${miracle.transaction.tcc.coordinatorThreadMax}")
     ...
     * 任务调度线程大小
     */
	@Value("${miracle.transaction.tcc.scheduledThreadMax}")
    private int scheduledThreadMax = Runtime.getRuntime().availableProcessors() << 1;

    /**
     * 调度时间周期 单位秒
     */
	@Value("${miracle.transaction.tcc.scheduledDelay}")
    private int scheduledDelay = 60;

    /**
     * 最大重试次数
     */
	@Value("${miracle.transaction.tcc.retryMax}")
     ...
     */
	@Value("${miracle.transaction.tcc.recoverDelayTime}")
     ...
     */
	@Value("${miracle.transaction.tcc.repositoryType}")
    private String repositoryType = "db";
	
	@Value("${miracle.transaction.tcc.dbType}")
    private String dbType = "mysql";
	
	@Value("${miracle.transaction.tcc.datasourceName}")
	private String datasourceName = "tcc";
	
	/**
	 * rpc等传递的线程上下文编辑处理类
	 */
	@Value("${miracle.transaction.tcc.threadContextLocalEditorClass}")
	private String threadContextLocalEditorClass = "com.miracle.common.transaction.api.ThreadContextLocalEditor";
